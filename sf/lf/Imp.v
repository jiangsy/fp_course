...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BEq e1 e2 n1 n2: (e1 ==> n1) -> (e2 ==> n2) -> (BEq e1 e2 ==>b (n1 =? n2))
  | E_BLe e1 e2 n1 n2: (e1 ==> n1) -> (e2 ==> n2) -> (BLe e1 e2 ==>b (n1 <=? n2))                    
  | E_BNot eb1 b1 : (eb1 ==>b b1) -> (BNot eb1 ==>b (negb b1))
  | E_BAnd eb1 eb2 b1 b2 : (eb1 ==>b b1) -> (eb2 ==>b b2) -> (BAnd eb1 eb2 ==>b andb b1 b2)     
...
Proof.
  ...
  - induction H; simpl; try auto.
    ...
    + rewrite IHbevalR. reflexivity.
    + rewrite IHbevalR1. rewrite IHbevalR2. reflexivity.
  - rewrite <- H. apply (bexp_ind (fun b => (forall bv, beval b = bv -> b ==>b bv))); intros.
    + inversion H0. constructor.
    + inversion H0. constructor.
    + unfold beval in H0. rewrite <- H0. constructor; apply aeval_iff_aevalR'; reflexivity.
    + unfold beval in H0. rewrite <- H0. constructor; apply aeval_iff_aevalR'; reflexivity.
    + simpl in H1. rewrite <- H1. constructor. apply H0. reflexivity.
    + simpl in H2. rewrite <- H2. constructor; auto.
    + reflexivity.
Qed.
(** [] *)
...
Proof.
  intros.
  apply E_Seq with (st':= (X !-> 0)). apply E_Asgn. reflexivity.
  apply E_Seq with (st':= (Y !-> 1; X !-> 0)). eapply E_Asgn. reflexivity.
  eapply E_Asgn. auto.
Qed.
(** [] *)
...
Proof.
  apply E_Seq with (st':= (Y !-> 0; X !-> 2)). eapply E_Asgn. auto.
  apply E_WhileTrue with (st':=(X !-> 1; Y !-> 2; Y !-> 0; X !-> 2)). auto.
  eapply E_Seq. eapply E_Asgn. eauto.
  eapply E_Asgn. eauto.
  eapply E_WhileTrue with (st':=(X !-> 0; Y !-> 3; X !-> 1; Y !-> 2; Y !-> 0; X !->2)). auto.
  eapply E_Seq. eapply E_Asgn. eauto.
  eapply E_Asgn. eauto.
...
Proof.
  ...
  - specialize (IHcontra2 Heqloopdef). inversion IHcontra2.
Qed.
...
Inductive no_whilesR: com -> Prop :=
 ...
 | E_NWAsgn x y: no_whilesR <{ x := y }>               
 | E_NWSeq c1 c2 : no_whilesR c1 -> no_whilesR c2 -> no_whilesR <{ c1 ; c2 }>
 | E_NWIf b ct cf : no_whilesR ct -> no_whilesR cf -> no_whilesR <{ if b then ct else cf end }>          
 (* FILL IN HERE *)
.
...
Proof.
  ...
  - induction c. 
    + constructor.
    + constructor.
    + inversion H. apply andb_true_iff in H1. inversion H1. apply IHc1 in H0. apply IHc2 in H2.
...
Theorem no_whiles_termination : forall c,
    no_whilesR c -> forall st, exists st', st =[ c ]=> st'
.
Proof.
  intros. generalize dependent st. induction H; intros.
  - exists st. constructor.
  - exists ( x !-> (aeval st y) ; st). constructor. auto.
  - specialize (IHno_whilesR1 st). inversion IHno_whilesR1 as [st'].
    specialize (IHno_whilesR2 st'). inversion IHno_whilesR2 as [st''].
    exists st''. eapply E_Seq; eauto.
  - destruct (beval st b) eqn:Heqb.
    + specialize (IHno_whilesR1 st). inversion IHno_whilesR1 as [st'].
      exists st'. apply E_IfTrue; auto.
    + specialize (IHno_whilesR2 st). inversion IHno_whilesR2 as [st'].
      exists st'. apply E_IfFalse; auto.
Qed.
...
Proof.
  ...
  induction e; intros; auto;
    (* 3 binary ops *) simpl; do 2 rewrite execute_app; rewrite IHe1; rewrite IHe2;  auto.
Qed. 
...
Theorem beval_sc_equiv : forall st b, beval_sc st b = beval st b.
Proof.
  intros. generalize st.
...
Theorem while_continue : forall b c st st' s,
  ...
  s = SContinue.
Proof.
  intros. remember <{ while b do c end }> as cWhile.
  induction H; inversion HeqcWhile.
  - auto.
  - subst. apply IHceval2 in HeqcWhile. assumption.
  - auto.
Qed.
...
Proof.
  ...
  - specialize (IHceval2 HeqcWhile H0). inversion IHceval2 as [st''']. exists st'''. assumption.
...
Proof.
  ...
  - (* Seq *) inversion Hc1; inversion Hc2; subst.
    + specialize (IHc1 st st' st'0 SContinue SContinue H1 H8). inversion IHc1. subst.
      specialize (IHc2 st'0 st1 st2 s1 s2 H5 H12). assumption.
    + subst. specialize (IHc1 st st' st2 SContinue SBreak H1 H11). inversion IHc1. discriminate.
    + specialize (IHc1 st st1 st'0 SBreak SContinue H4 H7). inversion IHc1. discriminate.
    + specialize (IHc1 st st1 st2 SBreak SBreak H4 H10). assumption.
  - destruct (beval st b); inversion Hc1; inversion Hc2; subst; try rewrite H5 in H13; try discriminate.
    + specialize (IHc1 st st1 st2 s1 s2 H6 H14). assumption.
    + specialize (IHc2 st st1 st2 s1 s2 H6 H14). assumption.
    + specialize (IHc1 st st1 st2 s1 s2 H6 H14). assumption.
    + specialize (IHc2 st st1 st2 s1 s2 H6 H14). assumption.
  - remember <{ while b do c end }> as c1While. induction Hc1; inversion Heqc1While; subst.
    ...
    + remember <{ while b do c end }> as c2While. induction Hc2; inversion Heqc2While; subst.
      * rewrite H in H0. discriminate.
      * specialize (IHc st st'0 st' SContinue SContinue Hc2_1 Hc1_1).
        inversion IHc. apply IHHc1_2. auto. rewrite H1 in Hc2_2. assumption.
      * specialize (IHc st st'0 st' SBreak SContinue Hc2 Hc1_1). inversion IHc. discriminate.
    + remember <{ while b do c end }> as c2While. induction Hc2; inversion Heqc2While; subst.
      * rewrite H in H0. discriminate.
      * specialize (IHc st st'0 st' SContinue SBreak Hc2_1 Hc1). inversion IHc. discriminate.
