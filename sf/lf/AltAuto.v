...
(** This chapter gives an alternative treatment of Coq automation that
    ...
    *)
    ...
    Ltac re_opt_match_empty := inversion IH2; try rewrite app_nil_r; auto.
    ...
    Proof.
      ...
      induction M
        as [| x'
            | s1 re1 s2 re2 Hmatch1 IH1 Hmatch2 IH2
            | s1 re1 re2 Hmatch IH | re1 s2 re2 Hmatch IH
            | re1 | s1 s2 re1 Hmatch1 IH1 Hmatch2 IH2]; simpl; auto.
      - destruct re1; try solve [ destruct re2; auto; re_opt_match_empty ].
        + inversion IH1. simpl. destruct re2; auto.
      - destruct re1; try destruct re2; auto; try inversion IH.
      - destruct re1; try destruct re2; auto; try inversion IH.
      - destruct re1; auto.
      - destruct re1; auto; inversion IH1. inversion IH2. apply MEmpty. 
    Qed.
    ...
    *)
    ...
    Hint Resolve le_plus_l : core.
    Hint Resolve le_plus_r : core.
    Hint Resolve leb_complete : core.
    Hint Resolve leb_false_rev : core.
    Hint Resolve plus_le_compat_l : core.
    Hint Resolve plus_le_compat_r : core.
    Hint Resolve napp_star : core.
    Hint Resolve napp_star' : core.
    
    Hint Rewrite app_nil_r : core_rw.
    ...
    Proof.
      ...
      -
        simpl. intros. rewrite app_length in H. apply add_le_cases in H. destruct H as [H1 | H2].
        + apply IH1 in H1. destruct H1 as [s2' [s3' [s4' [Heq [Hs3notempty [Hs234len Hs234pumping]]]]]]. auto.
          exists s2', s3', (s4' ++ s2). all : split_all; eauto.
          * rewrite Heq. app_assoc_eq.
          * intros. assert (s2' ++ (napp m s3') ++ s4' ++ s2 = (s2' ++ (napp m s3') ++ s4') ++ s2). { app_assoc_eq. }
            rewrite H. auto.
        + apply IH2 in H2. destruct H2 as [s2' [s3' [s4' [Heq' [Hs3'notempty [Hs'234len Hs'234pumping]]]]]].
          ...
          * apply leb_iff in Hpcre1. apply IH1 in Hpcre1.
            ...
            -- intros. assert (s2'' ++ napp m s3'' ++ s4'' ++ s2 = (s2'' ++ napp m s3'' ++ s4'') ++ s2). { app_assoc_eq. }
               rewrite H. auto.
          * exists (s1 ++ s2'), s3', s4'. all : split_all; auto.
            -- rewrite Heq'. app_assoc_eq.
      ...
      -
        simpl. intros.
        assert (H1: pumping_constant re1 <= length s1). { eauto. }
        apply IH in H1. destruct H1 as [s2' [s3' [s4' [Heq [Hs3notempty [Hs234len Hs234pumping]]]]]].
        exists s2', s3', s4'. all : split_all; eauto.
      -
        simpl. intros. 
        assert (H2: pumping_constant re2 <= length s2). { eauto. }
      ...
      -
        ...
        +
          ...
          **
             ...
             destruct (leb (pumping_constant re) (length s2)) eqn: Hpcre2; auto.
          ** 
             exists [], s1, s2. all : split_all; auto.
