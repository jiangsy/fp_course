...
Proof.
  ...
  split. intro.
  - 
    inversion H. unfold bequiv in Hb. rewrite Hb in H5. discriminate.
    subst. assumption.
  -
...
Proof.
  ...
  -
    ...
    { simpl in H5. apply negb_false_iff in H5. auto.}
    apply E_IfTrue; assumption.
Qed.
(** [] *)
...
Proof.
  intros.
  unfold cequiv. split; intro.
  - apply while_true_nonterm in H0. inversion H0. assumption.
  - apply while_true_nonterm in H0. inversion H0. unfold bequiv. simpl. intro. reflexivity.
Qed.    
(** [] *)
...
Proof.
  ...
  - inversion H0. subst. assert ((x !-> (aeval st' a); st') = st').
    { unfold aequiv in H.  rewrite <- H. apply t_update_same. }
    rewrite <- H1 at 2. apply E_Asgn. reflexivity.
  - inversion H0. subst. assert ((x !-> (aeval st a); st) = st).
    { unfold aequiv in H.  rewrite <- H. apply t_update_same. }
...
Definition equiv_classes : list (list com) :=
  [[prog_a; prog_d]; [prog_b; prog_e]; [prog_c; prog_f; prog_h]; [prog_g]; [prog_i]].
...
Proof.
  ...
  {
    unfold cequiv.
    intros c1 c1' c2 c2' st st' Hc1e Hc2e Hce.
    induction c1; inversion Hce; subst; apply E_Seq with (st':=st'0);
...
Proof.
  ...
  {
    unfold bequiv, cequiv.
    intros b b' c1 c1' c2 c2' st st' Hbe Hc1e Hc2e Hce.
    remember <{ if b then c1 else c2 end}> as cif eqn:Heqcif.
    induction Hce; inversion Heqcif; subst.
    - apply E_IfTrue. rewrite <- Hbe. assumption. apply Hc1e. assumption.
    - apply E_IfFalse. rewrite <- Hbe. assumption. apply Hc2e. assumption.
  }
  intros. split.
  - apply A; assumption.
  - apply A; apply sym_bequiv || apply sym_cequiv; assumption.
Qed.
(** [] *)
...
(* Consider the boolean property (P com) := "Have depth not more than 2"
    define the relation : com1 ~ com2 <-> P com1 = P com2
    [] *)
...
Proof.
  intros.
  induction a; inversion H; simpl;
    repeat (first [rewrite IHa1 | rewrite IHa2 | reflexivity | assumption]).
    (* try rewrite IHa1; try rewrite IHa2; try reflexivity; try assumption; try assumption. *)
...
Lemma not_used_subst_equal : forall x a1 a2 st,
  var_not_used_in_aexp x a1 ->
  aeval (x !-> aeval st a1; st) (subst_aexp x a1 a2) =
  aeval (x !-> aeval st a1; st) a2.
Proof.
  intros.
  induction a2; simpl; try (rewrite IHa2_1; rewrite IHa2_2); try reflexivity.
  - destruct (eqb_string x x0) eqn:Heqb.
    + rewrite aeval_weakening. rewrite eqb_string_true_iff in Heqb. rewrite Heqb.
...
Proof.
  unfold subst_equiv_property'. 
  intros. unfold cequiv. split; intros;
   inversion H0; subst; apply E_Seq with (st':=st'0); try assumption;
   inversion H3; inversion H6; subst.
  - rewrite <- not_used_subst_equal. apply E_Asgn. reflexivity. assumption.
  - rewrite not_used_subst_equal. apply E_Asgn. reflexivity. assumption.
Qed.
(* [] *)
...
Proof.
  unfold not. unfold cequiv. intros Contra.
  remember (X !-> 1) as st.
  assert (st =[ skip ]=> st). {apply E_Skip.}
  apply Contra in H. 
...
Proof.
  left.
  unfold cequiv. unfold pXY. unfold pYX. split; intros;
   inversion H; inversion H2; inversion H5; subst.
  - apply E_Seq with (st':= Y !-> n0; st).
    apply E_Havoc. rewrite t_update_permute. apply E_Havoc.
...
Ltac string_neq :=
  unfold not; intro Contra_string_eq; inversion Contra_string_eq.

Theorem ptwice_cequiv_pcopy :
  cequiv ptwice pcopy \/ ~cequiv ptwice pcopy.
Proof.
  right.
  unfold not. unfold cequiv.  intro Contra.
  remember (Y !-> 1; X !-> 0) as st'1.
  remember (Y !-> 0; X !-> 0) as st'2.

  assert (H1 : empty_st =[ ptwice ]=> st'1).
  {apply E_Seq with (st' := (X !-> 0)). apply E_Havoc. rewrite Heqst'1.  apply E_Havoc.}
  assert (H2 : empty_st =[ pcopy ]=> st'2).
  {apply E_Seq with (st' := (X !-> 0)). apply E_Havoc. rewrite Heqst'2. apply E_Asgn. simpl. apply t_update_eq. }

  apply Contra in H1. inversion H1. inversion H6. subst.
  assert ((Y !-> aeval st' X; st') Y = (Y !-> 1; X !-> 0) Y). { rewrite H10; reflexivity. }
  repeat rewrite t_update_eq in H.
  assert ((Y !-> aeval st' X; st') X = (Y !-> 1; X !-> 0) X). { rewrite H10; reflexivity. }
  do 2 rewrite t_update_neq in H0. rewrite t_update_eq in H0. simpl in H. rewrite H in H0.
  inversion H0. auto.
  string_neq. string_neq. string_neq. 
Qed.
(** [] *)
...
Proof.
  intros. split.
  - generalize dependent m. induction n; intro m; destruct m.
    + reflexivity.
    + intros. inversion H.  
    + intro H. inversion H.
    + intros. apply f_equal. apply IHn. simpl in H. apply H.
  -
    intros. rewrite H.
    apply eqb_refl.
Qed.
...
Lemma p1_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p1 ]=> st'.
Proof.
  unfold not. unfold p1. intros st st' Hstneq0. intro Contra.
  remember <{ while ~ X = 0 do havoc Y; X := X + 1 end }> as cw eqn:Heqcw.
  induction Contra; inversion Heqcw; subst. clear Heqcw. 
  -
    simpl in H. apply negb_false_iff in H. apply eqb_true in H.
    apply Hstneq0 in H. inversion H.  
  -
    assert  (Hrefl : <{ while ~ X = 0 do havoc Y; X := X + 1 end }> =
             <{ while ~ X = 0 do havoc Y; X := X + 1 end }>). { reflexivity. }
    apply IHContra2 in Hrefl as Contra. inversion Contra.
    inversion Contra1. inversion H2. inversion H5. subst. intro Hevalx.
    rewrite t_update_eq in Hevalx. simpl in Hevalx. rewrite t_update_neq in Hevalx.
    + destruct (st X) in Hevalx. inversion Hevalx. inversion Hevalx.
    + string_neq.
Qed.
...
Lemma p2_may_diverge : forall st st', st X <> 0 ->
  ~ st =[ p2 ]=> st'.
Proof.
  unfold not. unfold p2. intros st st' Hstneq0. intro Contra.
  ...
  -
    ...
    apply Hstneq0 in H. inversion H.
  -
    assert (Hrefl: <{ while ~ X = 0 do skip end }>
                   = <{ while ~ X = 0 do skip end }>). { reflexivity. }
    apply IHContra2 in Hrefl as Contra. inversion Contra.
    inversion Contra1. subst. apply Hstneq0.
Qed.
(** [] *)
...
Proof.
  intros. unfold cequiv. split; intros; inversion H; subst.
  -
    unfold p1 in H. unfold p2. inversion H; subst. apply E_WhileFalse. assumption.
    rewrite H2 in H4. inversion H4.
  -
    apply p1_may_diverge in H. inversion H. simpl in H2. unfold not. intro.
    apply eqb_true in H0. rewrite H0 in H2. inversion H2.
  -
    unfold p2 in H. unfold p2. inversion H; subst. apply E_WhileFalse. assumption.
    rewrite H2 in H4. inversion H4.
  -
    apply p2_may_diverge in H. inversion H. simpl in H2. unfold not.
    intro. apply eqb_true in H0. rewrite H0 in H2. inversion H2.
Qed.
(** [] *)
...
Proof.
  unfold not, cequiv. intro Contra.
  remember (X !-> 1) as st0.
  remember (Z !-> 0; X !-> 0; Z !-> 1; X !-> 1) as st1.
  remember (Z !-> 1; X !-> 0; X !-> 1) as st2.
  assert (H1: st0 =[ p3 ]=> st1). {
    unfold p3. rewrite Heqst0.
    apply E_Seq with (st':=(Z !-> 1; X !-> 1)).
    apply E_Asgn. auto.
    apply E_WhileTrue with (st':=(Z !-> 0; X !-> 0; Z !-> 1; X !-> 1)). auto.
    apply E_Seq with (st':= (X !-> 0; Z !-> 1; X !-> 1)); apply E_Havoc.
    rewrite Heqst1. apply E_WhileFalse. auto.
  }
  apply Contra in H1.
  inversion H1. subst. inversion H5. subst.
  simpl in H6.
  assert (1 = 0).
  { replace 1 with ((Z !-> 1; st') Z) at 1.
    replace 0 with ((Z !-> 0; X !-> 0; Z !-> 1; X !-> 1) Z) at 2.
    rewrite H6. auto.
    apply t_update_eq.
    apply t_update_eq.
  }
...
Proof.
  unfold cequiv. unfold p5. unfold p6. split; intros.
  -
    remember <{while ~ X = 1 do havoc X end }> as cwhile eqn:Heqcwhile.
    induction H; inversion Heqcwhile; subst.
    +
      simpl in H. apply negb_false_iff in H. apply eqb_true in H.  
      assert (st = (X !-> 1; st)).
      {
      ...
      rewrite H0 at 2. apply E_Asgn.
      simpl. reflexivity.
    + inversion H0. subst.
      apply IHceval2 in Heqcwhile. inversion Heqcwhile. subst. simpl. simpl in Heqcwhile.
      rewrite t_update_shadow. apply E_Asgn.
      auto.
  - inversion H. subst. simpl.
    destruct (st X =? 1) eqn:Heqstx0.
    + apply eqb_true in Heqstx0. rewrite <- Heqstx0 at 2.
      rewrite t_update_same.
      apply E_WhileFalse.
      simpl. apply negb_false_iff. apply eqb_true. assumption.
...
Notation "'for'  ( c1 ';;' b ';;' c2 ) c3" :=
         (CFor c1 b c2 c3)
           (in custom com at level 89, c1 at level 99, b at level 99, c2 at level 99, c3 at level 99).
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_For: forall st st' st'' b c1 c2 c3,
      ...
      st  =[ for (c1 ;; b ;; c2) c3 ]=> st''
where "st =[ c ]=> st'" := (ceval c st st').

Definition cequiv (c1 c2 : com) : Prop := forall st st' : state,
  st =[ c1 ]=> st' <-> st =[ c2 ]=> st'.

Theorem for_while_cequiv : forall b c1 c2 c3,
  cequiv <{ c1 ; while b do (c3 ; c2) end }>
         <{ for (c1 ;; b ;; c2) c3 }>.
Proof.
  unfold cequiv. split; intros.
  - inversion H. subst.
    apply E_For with (st':=st'0); assumption.
  - inversion H. subst.
...
Lemma neqb_string_symm: forall (x y: string), x <> y <-> y <> x.
Proof.
  unfold not. intros; split; intros Contra Heqxy; symmetry in Heqxy; apply Contra in Heqxy; inversion Heqxy.
Qed.
...
Proof.
  assert (A:  forall l1 l2 a1 a2 st st',
      l1 <> l2 -> var_not_used_in_aexp l1 a2 ->
      var_not_used_in_aexp l2 a1 ->
      st =[ l1:=a1; l2:=a2 ]=> st' ->
      st =[ l2:=a2; l1:=a1 ]=> st'). 
  {
    ...
    apply E_Asgn. reflexivity.
    assert (forall l, (l2 !-> aeval (l1 !-> aeval st a1; st) a2; l1 !-> aeval st a1; st) l =
                 (l1 !-> aeval (l2 !-> aeval st a2; st) a1; l2 !-> aeval st a2; st) l).
    {
      intro l. destruct (eqb_string l l1) eqn:Heqll1.
      -
        apply eqb_string_true_iff in Heqll1. subst. rewrite t_update_neq.
        do 2 rewrite t_update_eq. rewrite aeval_weakening; auto. auto.
      -
        destruct (eqb_string l l2) eqn:Heqll2.
        +
          apply eqb_string_true_iff in Heqll2. subst. apply eqb_string_false_iff in Heqll1.
          rewrite t_update_eq. rewrite t_update_neq.
          rewrite t_update_eq. apply aeval_weakening. assumption. assumption.
        +
          apply eqb_string_false_iff in Heqll1. apply eqb_string_false_iff in Heqll2.
          apply neqb_string_symm in Heqll1. apply neqb_string_symm in Heqll2.
  ...
  unfold cequiv. split; intros.
  -
    apply A; try assumption.
  -
    apply A; try assumption. apply neqb_string_symm. assumption.
Qed.
(** [] *)
...
Proof.
  assert (A: forall x, (x !-> 0) = (x !-> 1) -> 0 = 1).  {
    intros. 
    replace 0 with ((x !-> 0) x) at 1.
    replace 1 with ((x !-> 1) x) at 1.
    rewrite H. reflexivity.
    apply t_update_eq. apply t_update_eq. }
  unfold not. unfold capprox. unfold c3. unfold c4. 
  split; intros;
  remember (X !-> 0) as st1;
  remember (X !-> 1) as st2;
  assert (H1 : empty_st =[ X := 0 ]=> (X !-> 0));
  assert (H2 : empty_st =[ X := 1 ]=> (X !-> 1));
  try (apply E_Asgn; auto);
    try (apply H in H1 as Contra || apply H in H2 as Contra); inversion Contra; subst; simpl in H6.
  - symmetry in H6. apply A in H6. inversion H6.
  - apply A in H6. inversion H6.
Qed.
...
Proof.
  ...
  destruct (H st) as [st' Hexist].
  exists st'. apply H0. assumption.
Qed.
