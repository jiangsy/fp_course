...
Section Part_1_Martin_Lof_Type_Theory.
  ...
  Section Exercise_1_2.
    ...
    Definition double : nat -> nat :=  nat_rect (fun _ => nat) O (fun _ res => S (S res)).
  ...
  Section Exercise_1_3.
    ...
    Definition halve : nat -> nat := fun x => fst ((nat_rect (fun _ => (nat * Bool)) (O, true)
                                                     (fun _ res =>
                                                        match (snd res) with
                                                        |true => (fst (res), false)
                                                        |false => (S (fst res), true)
                                                        end
                                                       )) x).
   Eval compute in halve 10.
   Eval compute in halve 9. 
