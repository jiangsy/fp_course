...
data NatMax : Nat -> Set where  
   fromNat   : (n : Nat) -> NatMax n
   fromNatMax : {n : Nat} -> NatMax n -> NatMax (suc n)

buildNatMax : {m : Nat} -> (n : Nat) -> (m >= n) -> NatMax m
buildNatMax {m} n m>=n with difference m n m>=n 
... | d , m=n+d rewrite m=n+d = buildNatMaxHelp n d
  where
    buildNatMaxHelp : (n d : Nat) -> NatMax (n +N d)
    buildNatMaxHelp n zero = fromNat (n +N zero)
    buildNatMaxHelp n (suc d) rewrite suc-+N n d = fromNatMax (buildNatMaxHelp n d)

getNat : {m : Nat} -> NatMax m -> Nat
getNat (fromNat a) = a
getNat (fromNatMax x) = getNat x
...
record Application (wh : Nat * Nat) : Set where
  ...
  field
    ...
    paintMe       : Painting wh
    -- see difference or Vec X n?
    -- Nat n : n -> Nat n, Nat n -> Nat (suc n)
    cursorMe      : (NatMax (fst wh)) * (NatMax (snd wh))  -- x,y coords
    -- cursorMe      : Nat * Nat
...
appPaint (_ , app) =
  ...
  +L (goRowCol (getNat (snd xy)) (getNat (fst xy)) ,- [])
  where
    p  = paintMe app
    xy = cursorMe app
...
cursorMe (rectApp c (w , h)) = buildNatMax w (refl->= w) , buildNatMax h (refl->= h)
